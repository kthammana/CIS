...
# 1a. 3D points, rotations, and frame transformations
...
'''
def main():
    # Point b in Frame B at (1,1,0.5)
    point_b = Point3d("B",1,1,0.5)
    # Point a in Frame A at (0.2,0.4,0.3)
    point_a = Point3d("A",0.2,0.4,0.3)

    # Frames A and B defined w.r.t. some scanner (F_B <- F_0 -> F_A)
    R_A = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
    p_A = Point3d()
    frame_A = Frame("A",R_A,p_A,["0"])
